#!/usr/bin/env python3
"""
SynthRAD2025 Task 1 — 3D CWDM Training (ablation vs VS-DDPM)

Identical training loop to train_vs_ddpm.py; only the backbone model differs
(WavUNetModel3D with 3D Haar DWT/IDWT instead of nearest-neighbour sampling).

Usage:
    python training/train_cwdm.py --config training/configs/cwdm_3d.yaml --fold 0
    python training/train_cwdm.py --config training/configs/cwdm_3d.yaml --fold 0 \
        --resume /pvc/checkpoints/cwdm_3d/fold0_best.pth
"""
...
from src.models.cwdm_3d import (
    WavUNetModel3D,
    build_cwdm_and_diffusions,
    VARIABLE_T_VALUES,
...
@torch.no_grad()
def validate(model, diffusions, val_cases, device, val_steps, n_val_cases, patch_size):
    model.eval()
    all_mae, all_ssim = [], []
...
def train(cfg: dict, fold: int, resume: str = None):
    ...
    print(f"WavUNetModel3D parameters: {n_params:.1f}M")
...
def main():
    ...
    print(f"Training CWDM 3D  |  config: {args.config}  |  fold: {fold}")
