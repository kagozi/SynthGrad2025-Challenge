#!/usr/bin/env python3
"""
SynthRAD2025 Task 1 — Swin-UNETR 3D Inference (single or ensemble)

Loads one or more Swin-UNETR checkpoints, averages normalised predictions via
sliding window inference, saves flat submission files as:
    {output_dir}/sct_{case_id}.mha
    {output_dir}/../{output_dir_name}.zip

Usage (single fold):
    python inference/predict_swin.py \
        --checkpoints /pvc/checkpoints/swin_unetr/fold0_best.pth \
        --input_dirs \
            /pvc/data/synthRAD2025_Task1_Val_Input/Task1 \
            /pvc/data/synthRAD2025_Task1_Val_Input_D/Task1 \
        --output_dir /pvc/submissions/swin_fold0

Usage (ensemble):
    python inference/predict_swin.py \
        --checkpoints \
            /pvc/checkpoints/swin_unetr/fold0_best.pth \
            /pvc/checkpoints/swin_unetr/fold1_best.pth \
            ... \
        --output_dir /pvc/submissions/swin_ensemble
"""
...
from training.train_swin import build_model
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[SwinUNETR3D, dict]:
...
def main(args):
    ...
    print(f"[Swin Infer] Device: {device}")

    print(f"[Swin Infer] Loading {len(args.checkpoints)} checkpoint(s)...")
    ...
    overlap       = ic["overlap"]
    mode          = ic["mode"]
    print(f"[Swin Infer] roi_size={roi_size}  sw_batch={sw_batch_size}  overlap={overlap}")

    all_cases = build_case_list(args.input_dirs)
    print(f"[Swin Infer] Cases found: {len(all_cases)}")
    ...
    print(f"\n[Swin Infer] Done: {ok} OK, {err} errors")
    print(f"[Swin Infer] Output: {output_dir}")

    zip_path  = output_dir.parent / f"{output_dir.name}.zip"
    mha_files = sorted(output_dir.glob("sct_*.mha"))
    print(f"[Swin Infer] Zipping {len(mha_files)} files → {zip_path}")
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for f in mha_files:
            zf.write(f, f.name)
    print(f"[Swin Infer] Zip ready: {zip_path}  ({zip_path.stat().st_size/1e9:.2f} GB)")
